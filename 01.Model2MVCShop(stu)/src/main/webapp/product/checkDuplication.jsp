...
window.onload = function(){
	document.getElementById("prodNo").focus();
	document.getElementById("prodNo").onkeydown = function(){
...
function fncCheckDuplication() {
	// Form 유효성 검증
	if(document.detailForm.prodNo.value != null && document.detailForm.prodNo.value.length >0) {
		
	    document.detailForm.action='/checkDuplication.do';
	    document.detailForm.submit();
	}else {
		alert('상품번호는 반드시 입력하셔야 합니다.');
	}
	document.getElementById("prodNo").focus(); 
}
...
function fncUseId() {
	if(opener) {
		opener.document.detailForm.prodNo.value = "<%= prodNo %>";
	}
	window.close();
}
-->
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<%
						if(result) {
					%>
						<input type="text" name="prodNo" id="prodNo" value="<%=prodNo %>" class="ct_input_g" style="width:100px; height:19px"  maxLength="20" />
					<%
						}else {
					%>			
						<input type="text" name="prodNo" id="prodNo" class="ct_input_g" style="width:100px; height:19px"  maxLength="20" />
					<%
						}
					%>		
					</td>
